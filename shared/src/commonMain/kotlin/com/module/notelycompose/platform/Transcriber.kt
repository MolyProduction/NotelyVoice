...
expect class Transcriber {
    // Incremented at the start of every initialize() call. Returned from initialize()
    // so the caller captures the token atomically with the bump. ViewModels pass that
    // token back to finish(), so a stale finish() from a previous ViewModel session
